...
const DETAIL_KEYS_EN_AGREGADO = new Set([
  ...
  // top_banco / top_efectivo: usados en /caja/flujo-total para listar
  // top conceptos por categoría — son detalle itemizado.
  'top_banco', 'top_efectivo',
]);
...
function _walk(node, seen) {
  ...
  if (Array.isArray(node)) {
    // Filtrar SOLO movimientos individuales sensibles. Los agregados
    // (categorías con codigo sensible) NO se filtran — se procesan
    // abajo en el branch de objeto para limpiarles las keys de detalle.
  ...
  if (esAgregadoSensible(node)) {
    // El agregado se queda con total/n_movs/pct/codigo/nombre_display.
    // Las keys de detalle (proveedores/movimientos/top_*) se vacían.
...
module.exports = {
  ...
  sanitizeForNonAdmin, jsonSanitizerMiddleware,
};
