...
function limpiarDetalleAgregadoSensible(o) {
  ...
  // Forzar puede_drilldown=false para no-admin en cat sensible. Override
  // siempre — si el backend lo devolvió true por defecto, lo pisamos
  // acá. Esto evita que el frontend haga fetch y caiga en 403 con
  // "Error: Forbidden..." crudo. El frontend usa este flag para
  // mostrar 🔒 y bloquear la entrada sin tocar la red.
  o.puede_drilldown = false;
  // Flag explícito de "esta es una cat sensible bloqueada", útil para
  // que el frontend distinga "cat normal no drilleable" de "cat
  // sensible bloqueada por rol".
  o.es_sensible = true;
}
