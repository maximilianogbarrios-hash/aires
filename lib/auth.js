...
async function findUserByEmail(email) {
  ...
  return one('SELECT id, email, password_hash, role FROM ab_users WHERE email=$1', [email.toLowerCase()]);
}
...
async function createUser({ email, password, role }) {
  if (!['admin', 'socio'].includes(role)) throw new Error('invalid role');
...
async function listUsers() {
  const { rows } = await query('SELECT id, email, role, created_at FROM ab_users ORDER BY created_at ASC');
  return rows;
}
