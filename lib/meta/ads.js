...
let _cacheMem = { ts: 0, payload: null, accId: null }; // cache L1 en memoria
...
// Fetch limpio de TODOS los ads del account con la query EXACTA
// confirmada en Graph Explorer. Si una página falla, devuelve lo que
// hay + error. NUNCA devuelve [] por error parcial.
const ADS_FIELDS =
...
// Normaliza referencias anidadas → planas. La Marketing API a veces
// devuelve `adset_id`/`campaign_id` planos, a veces los anida en
// `adset{id}`/`campaign{id}` según los expand que pidamos. Defensa:
// si falta el plano, copiamos desde el anidado. SIN esto, lookups por
// `a.adset_id` quedan undefined → ad invisible (causa probable del
// "0 ads" reportado).
...
async function fetchAllAds(accId, token) {
  // Cap alto: 20 páginas × 500 = 10k ads. Cuentas con 500+ ads existen
  // (es el caso real de Aires). Si la cuenta crece más, subir aquí.
  const res = await _paginate(`${accId}/ads`, { fields: ADS_FIELDS }, token, { maxPages: 20, pageSize: 500 });
...
async function buildAdsDashboard({ refresh = false } = {}) {
  ...
  // VERSION BUMP: v3 invalida cache vieja con estructura buggy
  // (campañas con 0 ads por agrupamiento incorrecto previo).
  // Subir este número cuando cambie el shape del payload.
  const cacheKey = `ads_dashboard:v3:${accId}`;
  ...
  const adsRes = await fetchAllAds(accId, token);
  ...
  if (missingBodyMediaIds.size > 0) {
    ...
    // Aplicar a cada ad (flat) Y a los ads del árbol por adset (apuntan al mismo
    // objeto creative en muchos casos, pero por defensa los recorremos ambos).
    const applyCaption = (a) => {
      if (a.creative && !a.creative.body && a.creative.ig_media_id) {
    ...
    for (const c of campaigns) {
      for (const a of c.ads || []) applyCaption(a);
      for (const s of c.adsets) for (const a of s.ads) applyCaption(a);
    }
  }
