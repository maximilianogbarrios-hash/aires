...
async function fetchAllAds(accId, token) {
  return _paginate(`${accId}/ads`, { fields: ADS_FIELDS }, token, { maxPages: 10, pageSize: 500 });
}
...
async function buildAdsDashboard({ refresh = false } = {}) {
  ...
  // Indexar — y crear "ghost" entities para huérfanos. Si un ad tiene
  // adset_id que no está en la lista de adsets, igual lo mostramos
  // creando un adset ghost (status UNKNOWN). Mismo para campañas.
  const campsById  = new Map();
  for (const c of campsRes.data) campsById.set(c.id, c);
  const adsetsById = new Map();
  for (const s of adsetsRes.data) adsetsById.set(s.id, s);

  const adsetsByCampaign = new Map();
  for (const s of adsetsRes.data) {
    if (!adsetsByCampaign.has(s.campaign_id)) adsetsByCampaign.set(s.campaign_id, []);
    adsetsByCampaign.get(s.campaign_id).push(s);
  }
  const adsByAdset = new Map();
  let ghostAdsetCount = 0, ghostCampaignCount = 0;
  for (const a of adsRes.data) {
    // Si el ad referencia un adset no traído → creo ghost.
    if (a.adset_id && !adsetsById.has(a.adset_id)) {
      const ghostAdset = {
        id: a.adset_id, name: '(adset ' + a.adset_id.slice(-6) + ')',
        campaign_id: a.campaign_id, status: 'UNKNOWN', effective_status: 'UNKNOWN',
        daily_budget: 0, lifetime_budget: 0,
        _ghost: true,
      };
      adsetsById.set(a.adset_id, ghostAdset);
      if (!adsetsByCampaign.has(a.campaign_id)) adsetsByCampaign.set(a.campaign_id, []);
      adsetsByCampaign.get(a.campaign_id).push(ghostAdset);
      ghostAdsetCount++;
    }
    // Si el ad referencia una campaña no traída → ghost.
    if (a.campaign_id && !campsById.has(a.campaign_id)) {
      const ghostCamp = {
        id: a.campaign_id, name: '(campaña ' + a.campaign_id.slice(-6) + ')',
        objective: null, status: 'UNKNOWN', effective_status: 'UNKNOWN',
        _ghost: true,
      };
      campsById.set(a.campaign_id, ghostCamp);
      ghostCampaignCount++;
    }
