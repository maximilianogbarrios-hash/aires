...
// Spend reciente (últimos 3d) agrupado por campaign_id, en UNA query
// a nivel cuenta. Devuelve Map<String(campaign_id), totalSpend3d>.
// Esto es la señal real para decidir si una campaña ENTREGA, vs solo
// "tiene el toggle prendido".
async function fetchRecentSpendByCampaign(accId, token) {
  // level=campaign + date_preset=last_3d → 1 row por campaña con su
  // spend agregado. Si time_increment se omite, viene agregado al
  // período entero (lo que queremos).
  const r = await graphGet(`${accId}/insights`, {
    level: 'campaign',
    date_preset: 'last_3d',
  ...
  for (const row of data) {
    ...
    if (!cid) continue;
    map.set(cid, (map.get(cid) || 0) + Number(row.spend || 0));
  }
  ...
  return { ok: r.ok, map, n: map.size, pages, error: r.ok ? null : r.json?.error?.message };
}
...
async function buildAdsDashboard({ refresh = false } = {}) {
  ...
  // VERSION BUMP cuando cambie el shape: v7 redefine `is_delivering`
  // con la SEÑAL REAL — toggle prendido + spend reciente > 0 +
  // presupuesto no agotado. Agrega `recent_spend_3d` y
  // `not_delivering_reason` por campaña.
  const cacheKey = `ads_dashboard:v7:${accId}`;
  ...
  // Spend reciente (3d) por campaña — la señal real de delivery.
  const recentSpendRes = await fetchRecentSpendByCampaign(accId, token);
  if (!recentSpendRes.ok) console.warn('[meta.ads] recent_spend fetch failed:', recentSpendRes.error);
  const recentSpendMap = recentSpendRes.map || new Map();
  ...
  // Clasificación REAL de "entregando ahora" — replica la columna
  // "Entrega" de Meta Ads Manager.
  // Una campaña ENTREGA si y solo si:
  //   1. Tiene ≥1 ad con effective_status === 'ACTIVE' (cadena prendida).
  //   2. Tuvo spend > 0 en los últimos 3 días (delivery real).
  //   3. Si tiene presupuesto LIFETIME, no está agotado (spend_30d < budget * 0.99).
  // not_delivering_reason: cuando NO entrega, derivamos el motivo del
  // effective_status del ad + budget/schedule para que el dueño entienda.
  const nowSec = Math.floor(Date.now() / 1000);
  for (const c of campaigns) {
    const cIdStr = String(c.id);
    const ads = c.ads || [];
    const hasActiveAd = ads.some((a) => a.effective_status === 'ACTIVE');
    const recentSpend3d = recentSpendMap.get(cIdStr) || 0;
    const ins = c.insights || {};
    const lifetimeBudget = c.effective_budget?.kind === 'lifetime' ? c.effective_budget.monto : null;
    const budgetExhausted = lifetimeBudget != null && lifetimeBudget > 0
                          && (ins.spend || 0) >= lifetimeBudget * 0.99;
    const stopTs = c.stop_time ? Math.floor(new Date(c.stop_time).getTime() / 1000) : null;
    const outOfSchedule = stopTs != null && stopTs < nowSec;
    c.recent_spend_3d = recentSpend3d;
    c.budget_exhausted = budgetExhausted;
    c.out_of_schedule = outOfSchedule;
    c.is_delivering = hasActiveAd && recentSpend3d > 0 && !budgetExhausted && !outOfSchedule;
    // Motivo de no-entrega — para mostrar en el frontend.
    if (!c.is_delivering) {
      if (budgetExhausted) {
        c.not_delivering_reason = `Presupuesto agotado (€${(ins.spend||0).toFixed(0)} de €${lifetimeBudget.toFixed(0)}).`;
      } else if (outOfSchedule) {
        c.not_delivering_reason = `Fuera de fechas (fin: ${c.stop_time?.slice(0,10) || '—'}).`;
      } else if (!hasActiveAd) {
        // Inspeccionar effective_status de los ads para identificar dónde se cortó.
        const statuses = new Set(ads.map((a) => a.effective_status).filter(Boolean));
        if (statuses.has('CAMPAIGN_PAUSED')) c.not_delivering_reason = 'Campaña pausada.';
        else if (statuses.has('ADSET_PAUSED')) c.not_delivering_reason = 'Conjunto de anuncios pausado.';
        else if (statuses.has('PAUSED')) c.not_delivering_reason = 'Anuncios pausados.';
        else if (statuses.has('PENDING_REVIEW')) c.not_delivering_reason = 'En revisión por Meta.';
        else if (statuses.has('DISAPPROVED')) c.not_delivering_reason = 'Anuncios rechazados por Meta.';
        else if (ads.length === 0) c.not_delivering_reason = 'Sin anuncios.';
        else c.not_delivering_reason = 'Ningún anuncio activo (status: ' + [...statuses].join(', ') + ').';
      } else if (recentSpend3d === 0) {
        c.not_delivering_reason = 'Toggle prendido pero sin entrega en los últimos 3 días (puede ser falta de saldo, audiencia agotada o problema de creative).';
      } else {
        c.not_delivering_reason = 'No entrega.';
      }
    }
  }
