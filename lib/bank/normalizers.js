...
const INTRA_GRUPO_KEYWORDS = [
  ...
  // Variantes sin "Aires" — algunos extractos del banco truncan el nombre.
  // Ej: "Traspaso: Aportacion/Prestamo Grupo Hostelero Sl A Raba Buildings Sl"
  // antes caía en FINANCIERO porque no matcheaba "Grupo Hostelero Aires".
  'Grupo Hostelero Sl',
  'Aires Murcia',
  'Aires Benidorm',
];

// Prefijos comunes de conceptos que MENCIONAN una sociedad del grupo en el
// texto pero NO son movimientos intra-grupo. Casos típicos:
//   - "Liquidacion Efectuada El 29/05/2026 A Aires Alicante 3 3.804..."
//     → cobro TPV del banco al local (ingreso operativo real)
//   - "ABONO TPV 0354... AIRES BURGER BAR MURCIA"
//     → idem en formato Sabadell
//   - "Comisiones 0354... AIRES ALICANTE"
//     → comisión bancaria del TPV (gasto operativo)
//   - "Recibo Herederos De Sanchez Ruiz, S L Nº Recibo ... Ref. Mandato Smart Aires..."
//     → recibo a proveedor real; el banco mete el nombre de la sociedad
//       como referencia de mandato pero el mov es un PAGO saliente
//   - "Transferencia [Inmediata] A Favor De Goldfrio Logística Sl Concepto Envio Aires Alicante..."
//     → pago saliente a proveedor (concepto explica destino del envío)
// Sin este guard `.includes('Aires Alicante')` marcaba ~300 movs históricos
// como INTRAGRUPO inflando "Total ingresos" ~€160k en mayo 2026 y €1.2M
// acumulado.
const FALSOS_POSITIVOS_INTRA_GRUPO_PREFIJOS = [
  /^liquidacion efectuada/i,
  /^abono tpv/i,
  /^comisiones /i,
  /^recibo /i,
  /^transferencia (?:inmediata )?a favor de /i,
];

function esIntraGrupo(concepto) {
  const c = String(concepto || '').toLowerCase();
  // Whitelist explícito de patrones operativos que mencionan al local
  // pero NO son traspasos entre cuentas/sociedades del grupo.
  for (const rx of FALSOS_POSITIVOS_INTRA_GRUPO_PREFIJOS) {
    if (rx.test(concepto || '')) return false;
  }
...
const PROVEEDORES_KEYWORDS = [
  ...
  // EQUIPAMIENTO — proveedores específicos con nombre propio para que cada
  // uno sea un slice independiente del donut. NO se colapsan en un único
  // "Mantenimiento y Equipamiento" como antes.
  { re: /ggm\s+gastro/i,                                                     proveedor: 'GGM Gastro',                  categoria: 'EQUIPAMIENTO' },
  { re: /\bamazon\b|\bamz\b\s*mktp|\bamzn\s*mktp|\bamzn\b/i,                  proveedor: 'Amazon',                      categoria: 'EQUIPAMIENTO' },
  { re: /\bikea\b/i,                                                         proveedor: 'IKEA',                        categoria: 'EQUIPAMIENTO' },
  { re: /maquinas\s+febal/i,                                                 proveedor: 'Maquinas Febal',              categoria: 'EQUIPAMIENTO' },
  { re: /argent\s*3d|argen\s*3d/i,                                            proveedor: 'Argent 3D',                   categoria: 'EQUIPAMIENTO' },
  { re: /\bworten\b|media\s*markt/i,                                          proveedor: 'Equipamiento Electrónico',    categoria: 'EQUIPAMIENTO' },
  { re: /\bthomann\b/i,                                                      proveedor: 'Equipamiento Audio',          categoria: 'EQUIPAMIENTO' },
  { re: /cocinas\s+industriales|inox\s+levante|ecoclima|fibraclim|maquinaria\s+hosteler|decoraciones?\s+decomaber|escoda/i, proveedor: 'Equipamiento Cocina y Clima', categoria: 'EQUIPAMIENTO' },

  // MANTENIMIENTO — gastos corrientes para mantener el local.
  ...
  { re: /\bbrico\s*depot\b/i,                                                proveedor: 'Brico Depot',                 categoria: 'MANTENIMIENTO' },
  { re: /obramat/i,                                                          proveedor: 'Obramat',                     categoria: 'MANTENIMIENTO' },
  { re: /\bviveros\b/i,                                                      proveedor: 'Viveros',                     categoria: 'MANTENIMIENTO' },
  { re: /saniagua/i,                                                         proveedor: 'Saniagua',                    categoria: 'MANTENIMIENTO' },
  { re: /todoelectrico|new\s+matelsa|\bmatelsa\b|electricas?\s+maisa/i,       proveedor: 'Materiales Eléctricos',       categoria: 'MANTENIMIENTO' },
  { re: /conduce\s+revel|muebles\s+rosillo|sklum|ductoaire|paypal\s*\*?temu|\btemu\b|aliexpress|\bobm\s+murcia\b|sumin\s+surec|materiales\s+cano|bolsemack|el\s+corte\s+electr|alcomar\s+herrega|discount_es|coop\s+electrica\s+benefica|rros\s+imagen|rios\s+imagen|timbrad[oa]s|rotulaci[oó]n|cartel(er[ií]a)?|\bferreteria/i, proveedor: 'Mantenimiento', categoria: 'MANTENIMIENTO' },
  ...
  // Amazon — categorizado como EQUIPAMIENTO con nombre canónico "Amazon"
  // (regla específica al inicio del bloque EQUIPAMIENTO arriba ya lo cubre;
  // este bloque queda como fallback genérico para variantes residuales).
  { re: /\bamazon\b|amazon\s+(es|eu|web|marketplace|seller|pago)/i,           proveedor: 'Amazon', categoria: 'EQUIPAMIENTO' },
];
...
function normalizarProveedor(concepto, categoriaOriginal) {
  ...
  // 3) Recibo X (corto)
  m = /^Recibo\s+(.{3,}?)(?:\s+Nº Recibo|\s+Ref\.|\s+Cod\.|[,.])/i.exec(c);
...
module.exports = { esIntraGrupo, normalizarProveedor, INTRA_GRUPO_KEYWORDS, FALSOS_POSITIVOS_INTRA_GRUPO_PREFIJOS };
