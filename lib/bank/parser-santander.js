...
// Shim de compat. El hash ahora vive en lib/bank/hash-mov.js y es
// estable cross-format (excluye codigo_banco / num_documento y normaliza
// la fecha a MIN(fecha, fecha_valor)). Mantengo la firma vieja en el export
// por si algún consumidor de tests lo llama, pero ignoramos los argumentos
// que ya no entran al hash.
function hashRow({ sociedad_id, fecha, fecha_valor, concepto, importe }) {
  return hashMovimiento({ sociedad_id, fecha, fecha_valor, concepto, importe });
}
...
function parseSantanderBuffer(buffer, { sociedad_id, banco = 'santander' }) {
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    mov.hash = hashMovimiento({ sociedad_id, fecha, fecha_valor, concepto, importe });
