...
const CATEGORIAS_GASTO = [
  ...
  // Ronda 5 (2026-05-21): Sueldos Dirección separado de Nóminas, Equipamiento
  // separado de Mantenimiento, Préstamos separado de Financiero, Otros Gastos
  // específico para servicios externos (Radius, etc.).
  'NOMINAS_DIRECCION', 'EQUIPAMIENTO', 'PRESTAMOS', 'OTROS_GASTOS',
  // Ronda 6 (2026-05-21): gastos de dirección (Revel/Créditos) y vehículos
  // (Stellantis/leasing/renting) separados del bucket genérico.
  'GASTOS_DIRECCION', 'GASTOS_VEHICULOS',
];
...
function categorizarIngreso(concepto) {
  // Pre-INTRAGRUPO: traspasos/préstamos ENTRANTES desde sociedades del grupo
  // tienen que categorizarse como INTRAGRUPO antes de aplicar reglas más
  // específicas (la regla "transferencia de" los pillaba como
  // INGRESO_TRANSFERENCIA aunque vinieran de Aires Alicante / Smart Aires /
  // etc.). El detector esIntraGrupo ahora excluye los falsos positivos
  // operativos (Liquidacion TPV, ABONO TPV, Comisiones) que mencionan al
  // local pero no son intra-grupo.
...
const REGLAS_FIN_Y_MANT = [
  ...
  // EQUIPAMIENTO (renovaciones, locales nuevos, maquinaria, mobiliario,
  // electrónica) — categoría separada de MANTENIMIENTO para distinguir
  // gastos de inversión / re-equipamiento de gastos corrientes.
  { re: /ggm\s+gastro|\bikea\b|media\s*markt|\bworten\b|maquinas\s+febal|argent\s*3d|argen\s*3d|\bthomann\b|cocinas\s+industriales|inox\s+levante|ecoclima|fibraclim|maquinaria\s+hosteler|decoraciones?\s+decomaber|escoda/i, cat: 'EQUIPAMIENTO' },
  { re: /\bamazon\b|\bamz\b\s*mktp|\bamzn\s*mktp|\bamzn\b|amazon\s+(es|eu|web|marketplace|seller|pago)/i, cat: 'EQUIPAMIENTO' },
  // Mantenimiento: Leroy, ferretería, técnicos, obra, materiales eléctricos.
  ...
  { re: /\bmantenimiento\b|\breparacion\b|\bfontaneria\b|electricidad\s+(instalac|reparac)|tornilleria|sklum|conduce\s+revel|muebles\s+rosillo|ductoaire/i, cat: 'MANTENIMIENTO' },
  { re: /\btecnico\s+frigorif|aire\s+acondic|climatizacion|extincion\s+incend|tienda\s+animales/i, cat: 'MANTENIMIENTO' },
  // Materiales eléctricos, fontanería, viveros, compras corrientes
  { re: /new\s+matelsa|\bmatelsa\b|saniagua|todoelectrico|electricas?\s+maisa|obramat|sumin\s+surec|aliexpress|\bobm\s+murcia\b|el\s+corte\s+electr|\bviveros\b|discount_es|\bsanitarios?\b\s+y\s+griferia|electrod[ó]mestic|coop\s+electrica\s+benefica|bolsemack|materiales\s+cano|\bpaypal\s*\*?temu|\btemu\b|alcomar\s+herrega/i, cat: 'MANTENIMIENTO' },
