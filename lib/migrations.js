...
const MIGRATIONS = [
  ...
  {
    id: 32,
    name: 'presup_costos_uniq_sin_user_id_y_cleanup_dups',
    // Fix del bug "no persiste": el UNIQUE original incluía user_id, que
    // en este flujo siempre es NULL. En Postgres NULL ≠ NULL en UNIQUE
    // (sin NULLS NOT DISTINCT, PG15+), entonces el ON CONFLICT del PUT
    // nunca matcheaba y cada save insertaba una fila NUEVA. Resultado:
    // 315 filas para 35 cats (9 saves × 35 cats), y el GET con ORDER BY
    // monto DESC dejaba al frontend con el monto más chico (último set
    // wins del Map).
    //
    // Cleanup: dejar solo la fila más reciente por (anio, mes, scope,
    // categoria). DROP del UNIQUE viejo + ADD nuevo SIN user_id.
    // user_id queda en la tabla como nullable por compatibilidad futura;
    // si en algún momento se quiere por-usuario, se agrega de vuelta al
    // UNIQUE con NULLS NOT DISTINCT (PG15+) o COALESCE.
    up: `
      -- 1) Limpiar duplicados: dejar solo la fila con updated_at más reciente.
      DELETE FROM ab_presupuesto_costos_categoria a
            USING ab_presupuesto_costos_categoria b
       WHERE a.anio = b.anio
         AND a.mes  = b.mes
         AND a.scope = b.scope
      ...
      -- 2) Drop del UNIQUE viejo (nombre default que pone Postgres).
      DO $migr$
      DECLARE c text;
      BEGIN
        SELECT conname INTO c FROM pg_constraint
         WHERE conrelid = 'ab_presupuesto_costos_categoria'::regclass
           AND contype = 'u';
        IF c IS NOT NULL THEN
          EXECUTE 'ALTER TABLE ab_presupuesto_costos_categoria DROP CONSTRAINT ' || quote_ident(c);
        END IF;
      END
      $migr$;

      -- 3) UNIQUE nuevo, sin user_id.
      ALTER TABLE ab_presupuesto_costos_categoria
        ADD CONSTRAINT ab_presup_costos_uq UNIQUE (anio, mes, scope, categoria);
    `,
  },
  {
    id: 37,
    name: 'ab_simulador_local',
    // Parámetros persistentes del simulador de rentabilidad por local.
    // Cada fila = 1 local. Facturación (auto desde cierres TPV) puede
    // overridearse por escenario. El resto son inputs del dueño.
    // Pre-carga 4 locales con datos del prompt (Merced, Crevillente,
    // San Vicente, Chicken Thader) — idempotente por ON CONFLICT.
    up: `
      CREATE TABLE IF NOT EXISTS ab_simulador_local (
        local_id              VARCHAR(30) PRIMARY KEY,
        personal_ss           NUMERIC(12,2) NULL,
        alquiler              NUMERIC(12,2) NULL,
        suministros           NUMERIC(12,2) NULL,
        facturacion_override  NUMERIC(12,2) NULL,
        pct_mp                NUMERIC(5,2)  NOT NULL DEFAULT 38.00,
        pct_personal_evitable NUMERIC(5,2)  NOT NULL DEFAULT 100.00,
        notas                 TEXT NULL,
        updated_at            TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_by_email      TEXT NULL
      );
      INSERT INTO ab_simulador_local (local_id, personal_ss, alquiler, suministros, pct_mp, pct_personal_evitable)
      VALUES
        ('MURCIA_MERCED',  1730.00, 1326.00, 1142.00, 38.00, 100.00),
        ('CREVILLENTE',    3150.00, 1020.00,  750.00, 38.00, 100.00),
        ('SAN_VICENTE',    3920.00, 1423.00,  520.00, 38.00, 100.00),
        ('CHICKEN_THADER', 4000.00, 1920.00, 1010.00, 38.00, 100.00)
      ON CONFLICT (local_id) DO NOTHING;
    `,
  },
  {
    id: 36,
    name: 'ab_movimientos_es_extraordinario',
    // Flag para marcar movimientos como INGRESO/EGRESO EXTRAORDINARIO
    // (cobros atrasados, indemnizaciones, venta de activos, etc.).
    // Los extraordinarios quedan visibles pero SE EXCLUYEN del resultado
    // OPERATIVO — se reportan aparte para transparencia.
    // Pre-marcamos el mov Glovo €60.500 del 2026-06-12 (Smart Sabadell,
    // "Transferencia De Glovoapp Spain Platf. Slu ... /inv/14 2026")
    // que corresponde a una factura de abril cobrada en junio.
    up: `
      ALTER TABLE ab_movimientos
        ADD COLUMN IF NOT EXISTS es_extraordinario BOOLEAN NOT NULL DEFAULT FALSE;
      ALTER TABLE ab_movimientos
        ADD COLUMN IF NOT EXISTS extraordinario_motivo TEXT;
      CREATE INDEX IF NOT EXISTS idx_ab_movs_extraordinario
        ON ab_movimientos(es_extraordinario) WHERE es_extraordinario;
      -- Pre-marcado del cobro atrasado Glovo (factura /inv/14 2026 abril).
      UPDATE ab_movimientos
         SET es_extraordinario = TRUE,
             extraordinario_motivo = 'Factura Glovo /inv/14 2026 abril, cobrada en junio (atrasada)'
       WHERE sociedad_id = 'smart'
         AND banco = 'sabadell'
         AND fecha = '2026-06-12'
         AND importe > 60000 AND importe < 61000
         AND concepto ~* 'glovoapp'
         AND es_extraordinario = FALSE;
    `,
  },
  {
    id: 35,
    name: 'ab_meta_cache',
    // Cache server-side de respuestas Graph API. key = identificador
    // único de la consulta (ej. 'ig_dashboard:30d', 'ads_snapshot',
    // 'thumb:{mediaId}'). value = JSON (para datos) o base64 (para
    // imágenes — payload_b64). TTL definido por el caller; el cleanup
    // de expirados se hace al leer (lazy DELETE si stale).
    up: `
      CREATE TABLE IF NOT EXISTS ab_meta_cache (
      ...
      CREATE INDEX IF NOT EXISTS idx_ab_meta_cache_expires ON ab_meta_cache(expires_at);
    `,
  },
  {
    id: 34,
    name: 'ab_ads_backups',
    // Backups versionados del estado de la cuenta de Meta Ads (F3).
    // Cada backup es un snapshot JSON de campañas/adsets/ads con sus
    // métricas y configuración. Permite reconstruir si la agencia
    // borra/cambia algo en el panel de Meta. trigger_kind = 'manual'
    // (botón en UI) o 'auto' (scheduler futuro). Cap de retención
    // queda al user — sin auto-pruning por ahora.
    up: `
      CREATE TABLE IF NOT EXISTS ab_ads_backups (
        id              SERIAL PRIMARY KEY,
        ts              TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      ...
      CREATE INDEX IF NOT EXISTS idx_ab_ads_backups_ts  ON ab_ads_backups(ts DESC);
      CREATE INDEX IF NOT EXISTS idx_ab_ads_backups_acc ON ab_ads_backups(ad_account_id, ts DESC);
    `,
  },
  {
    id: 33,
    name: 'ab_meta_tokens',
    // Storage de tokens de Meta (Instagram Business / Facebook Page).
    // Valor encriptado AES-256-GCM (ver lib/meta/crypto.js). La clave
    // maestra vive en META_TOKEN_KEY (env var, NO hardcoded). `kind`
    // distingue entre tokens posibles: 'ig_access_token' (Page Token
    // para insights orgánicos), 'meta_user_token' (User Token con
    // scope ads_read/ads_management para Marketing API). UNIQUE por
    // kind: solo un token activo por tipo.
    up: `
      CREATE TABLE IF NOT EXISTS ab_meta_tokens (
        id              SERIAL PRIMARY KEY,
        kind            TEXT NOT NULL,
      ...
      CREATE INDEX IF NOT EXISTS idx_ab_meta_tokens_kind ON ab_meta_tokens(kind);
    `,
  },
  {
    id: 31,
