...
const PERMS = {
  ...
  // Caja/efectivo: lectura de movs y vista combinada con banco
  caja_view:       ['admin','socio','gerente'],
  ...
  ventas_upload:   ['admin'],
  // Editar costos y recetas de productos en /ventas/costos. Admin,
  // socio y gerente pueden modificar. Pedidos ve costos MP (no margen)
  // pero no edita. Personal sin acceso al tab Costos.
  ventas_costos_edit: ['admin','socio','gerente'],
  // Panel "Reglas de Proveedores" en /bancos (drag & drop). Crea
  // reglas globales en ab_reglas_normalizacion + reclasifica históricos.
  // Sólo admin y socio (en la práctica: Maxi + Dani).
  bancos_reglas_admin: ['admin','socio'],
  // Sub-permiso: clasificar con IA (vía OpenRouter). Incurre en costo
  // de tokens y requiere setear OPENROUTER_API_KEY. En la práctica =
  // Maxi (admin) + Dani (socio).
  bancos_reglas_ia:    ['admin','socio'],
  // Módulo de Meta Ads (Instagram + Facebook). F1 = ver dashboard IG
  // orgánico + manejo de token. F2/F3 (futuro) = pausar/budget/crear ads
  // y análisis IA. Visibilidad y acciones limitadas a admin + socio
  // (Maxi + Dani) por seguridad — manipular ads consume presupuesto real.
  meta_ads_view:   ['admin','socio'],
  meta_ads_admin:  ['admin','socio'],
};
...
const SUB_TABS_BANCOS = {
  ...
  cruce:       ['admin','socio'],
  // Flujo Anual: tabla mensual + comparador. Gerente lo ve sin el
  // desglose de cats sensibles (totales sí, sus categorías no — el
  // backend filtra GASTOS_DIRECCION/NOMINAS_DIRECCION/PRESTAMOS del
  // breakdown para no-admin).
  flujo:       ['admin','socio','gerente'],
  // Efectivo: caja histórica + análisis por sucursal/sociedad.
  efectivo:    ['admin','socio','gerente'],
  // Flujo Total: banco + efectivo unidos por origen/categoría
  // (con desglose y lista de pendientes de reclasificación).
  flujototal:  ['admin','socio','gerente'],
  // Simulador de rentabilidad y cierre de local. Admin/socio ven todo;
  // gerente ve solo lectura del panel (los inputs editables se
  // deshabilitan si no tiene bancos_reglas_admin — el PUT también
  // exige ese perm en el backend).
  simlocal:    ['admin','socio','gerente'],
};
