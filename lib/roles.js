...
const PERMS = {
  ...
  // Subida de extractos/cierres TPV: SÓLO admin (no socio, gerente ni administrativo).
  // Cierra el endpoint para evitar uploads accidentales o no autorizados.
  bancos_upload_admin: ['admin'],
  ...
  // Export de datos en formato descargable (CSV / Excel / PDF). Sólo admin
  // por ahora — cualquier endpoint nuevo que entregue archivos descargables
  // debe protegerse con requirePerm('export_w'). En el futuro puede habilitarse
  // por usuario desde /admin sin tocar código.
  export_w:        ['admin'],
};
